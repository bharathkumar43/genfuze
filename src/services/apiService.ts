...
class ApiService {
  ...
  async analyzeContentStructure(content: string, url?: string) {
    const res = await fetch(`${API_BASE_URL}/content/analyze-structure`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${localStorage.getItem('accessToken') || ''}`
    ...
    const json = await res.json();
    if (!res.ok) throw new Error('Failed to analyze content structure: ' + (json.error || res.status));
    return json;
  }
  ...
  async getAIVisibilityAnalysis(company: string): Promise<any> {
    return this.request(`/ai-visibility/${encodeURIComponent(company)}`);
  }
}
