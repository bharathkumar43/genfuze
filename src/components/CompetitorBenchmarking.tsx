...
import { BarChart3, Globe, TrendingUp, Target, AlertCircle, CheckCircle, Clock, Trash2 } from 'lucide-react';
...
const COMPETITOR_BENCHMARKING_KEY = 'competitor_benchmarking_state';

export function CompetitorBenchmarking({ competitorDomains: initialCompetitorDomains }: CompetitorBenchmarkingProps) {
  const location = useLocation();
  const stateUrls = location.state && Array.isArray(location.state.urls) ? location.state.urls : null;
  const [competitorDomains, setCompetitorDomains] = useState<string[]>(stateUrls || initialCompetitorDomains);
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-green-600 text-white px-4 py-2 rounded-lg">
          <BarChart3 className="w-5 h-5" />
          <span className="font-semibold">Benchmarking</span>
        </div>
      </div>
      ...
      {competitors.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            ...
            <p className="text-3xl font-bold text-blue-600">{getAverageScore(competitors)}%</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center gap-3 mb-2">
      ...
      {competitors.length > 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-6">Content Analysis</h3>
                ...
                <div key={index} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-semibold text-blue-700 underline break-all">{competitor.domain}</h4>
                  ...
                  {competitor.status === 'completed' && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="text-center">
                        ...
                        <p className="text-xl font-bold text-blue-600">{competitor.contentQuality}%</p>
                      </div>
                      <div className="text-center">
      ...
      {competitors.length === 0 && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-600 mb-6">Add competitor domains to start benchmarking your content</p>
          <div className="flex items-center justify-center gap-4">
            <input
              type="text"
              value={newDomain}
              onChange={(e) => setNewDomain(e.target.value)}
              placeholder="Enter URL or domain (e.g., example.com or https://example.com/page)"
            ...
            <button
              ...
              className="bg-primary text-white font-semibold px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50"
            >
              Add Competitor
            </button>
          </div>
